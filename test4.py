...
def initialize_tracks(measurements, doppler_threshold, range_threshold, firm_threshold, time_threshold, mode):
    ...
    return tracks, track_id_list, miss_counts, hit_counts, firm_ids, state_map, progression_states  # Return progression_states
        ...
        try:
            ...
            for track_id, track in enumerate(tracks):
                if track:
                    ...
                    current_state = state_map.get(track_id)
                    if current_state:
                        output += f"  Current State: {current_state}\n"
                    else:
                        output += f"  Current State: {progression_states[0]}\n"  # Default to 'Poss1'
