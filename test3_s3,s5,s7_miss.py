...
def initialize_tracks(measurements, doppler_threshold, range_threshold, firm_threshold, time_threshold, mode):
    ...
    for i, measurement in enumerate(measurements):
        ...
        if not assigned:
            ...
            state_map[new_track_idx] = 'Poss1'

        # Check miss counts and release the track if necessary
        for track_id in range(len(tracks)):
            if track_id in miss_counts:
                miss_counts[track_id] += 1 if not assigned else 0  # Increment miss count if not assigned

                # Get current state of the track
                current_state = state_map.get(track_id, 'Unknown')
                ...
                if miss_counts[track_id] >= miss_threshold:
                    tracks[track_id] = []  # Clear the track
                    release_track_id(track_id_list, track_id)
                    state_map.pop(track_id, None)  # Remove state from state_map

    return tracks, track_id_list, miss_counts, hit_counts, firm_ids, state_map
...
class TrackApp(QWidget):
    ...
    def execute_track_initialization(self):
        try:
            ...
            # Initialize tracks
            tracks, track_id_list, miss_counts, hit_counts, firm_ids, state_map = initialize_tracks(
            ...
            for track_id, track in enumerate(tracks):
                if track:
                    ...
                    output += f"  Current State: {state_map.get(track_id, 'Unknown')}\n"
    ...
    # Clear output in the QTextEdit box
